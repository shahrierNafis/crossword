...
class CrosswordCreator:
    ...
    def select_unassigned_variable(self, assignment):
        ...
        for var in self.crossword.variables - set(assignment):
            if (
                best is None
                or len(self.domains[var]) < len(self.domains[best])
                or (
                    len(self.domains[var]) == len(self.domains[best])
                    and len(self.crossword.neighbors(var))
                    > len(self.crossword.neighbors(best))
                )
            ):
                best = var
        return best
